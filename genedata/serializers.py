...
class GeneSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        """
        Update and return an existing `Gene` instance, given the validated data.
        """
        ...
        instance.start_codon = validated_data.get('start_codon', instance.start_codon)
        instance.ec = EC.objects.get(pk=self.initial_data.get('ec')['id'])
        instance.sequencing = Sequencing.objects.get(pk=self.initial_data.get('sequencing')['id'])
        instance.save()
        return instance
